...
if __name__ == '__main__':
    ...
    #                 drop_cnt = 0
    #                 stateful_function(file_name, writeback_ratio)

    # file = open("result.txt", "w")

    # for pkt_num in range(10000, 10001, 10000):
    #     for flow_num in range(100, 2001, 100):
    #         for zipfa in range(101, 121, 1):
    #             zipfa /= 100
    #             for writeback_ratio in range(5, 101, 5):
    ...
    def task(chunk, thread_index):
        for file_name in chunk:
            for writeback_ratio in range(5, 101, 5):
                value = stateful_function(file_name, writeback_ratio)
                with open(f"thread_{thread_index}_result.txt", "a") as file:  # 使用 'a'（追加模式）而不是 'w'（写入模式）
                    file.write(value)

    # 生成文件名列表
    file_names = [f'syn_{pkt_num}_{flow_num}_100000_{zipfa/100}' for zipfa in range(101, 121, 1) for flow_num in range(100, 2001, 100) for pkt_num in range(10000, 100001, 10000)]

    # 将文件名列表分块
    chunk_size = 100  # 每个线程处理的任务数量
    chunks = [file_names[i:i + chunk_size] for i in range(0, len(file_names), chunk_size)]

    # print(len(chunks[0]))

    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(task, chunk, i) for i, chunk in enumerate(chunks)]  # 将线程索引传递给任务函数
